...
namespace PaintIn3D
{
	...
	public static partial class P3dHelper
	{
		...
		public static Texture2D GetReadableCopy(RenderTexture renderTexture, TextureFormat format = TextureFormat.ARGB32, bool mipMaps = false)
		{
			if (renderTexture != null)
			{
				var texture   = new Texture2D(renderTexture.width, renderTexture.height, format, mipMaps, QualitySettings.activeColorSpace == ColorSpace.Linear);
				var oldActive = RenderTexture.active;
		...
		public static Texture2D GetReadableCopy(Texture texture, TextureFormat format = TextureFormat.ARGB32, bool mipMaps = false, int width = 0, int height = 0)
		{
			...
			if (texture != null)
			{
				...
				if (CanReadPixels(format) == true)
				{
					...
					newTexture = new Texture2D(width, height, format, mipMaps, QualitySettings.activeColorSpace == ColorSpace.Linear);
